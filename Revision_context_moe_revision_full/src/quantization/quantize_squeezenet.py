"""
Static INT8 Quantization for SqueezeNet 1.1.

Pipeline:
    1. Load SqueezeNet 1.1 (torchvision) + checkpoint weights
    2. Export → ONNX FP32
...
def build_model(num_classes: int = 8) -> nn.Module:
    """Create a fresh SqueezeNet 1.1 instance via torchvision."""
    weights = SqueezeNet1_1_Weights.IMAGENET1K_V1
    model = squeezenet1_1(weights=weights)
    # Modify output to 8 classes (same as src/models/pretrained_model/squeezenet.py)
