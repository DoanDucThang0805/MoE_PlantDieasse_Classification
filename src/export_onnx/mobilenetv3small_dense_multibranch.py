"""
Module for exporting MobileNetV3-Small Dense Multi-Branch Model to ONNX format.

This module provides functionality to export a trained MobileNetV3-Small model with
dense multi-branch expert architecture to ONNX (Open Neural Network Exchange) format,
enabling deployment on various inference engines and platforms.

The model architecture consists of:
- MobileNetV3-Small backbone (pre-trained)
- Multiple parallel expert branches (default: 4 experts)
- Each expert branch:
  * Linear(576 -> 512) + Hardswish + Dropout(0.2)
  * Linear(512 -> 8) [8 plant disease classes]
- Output aggregation: Average of all expert predictions
...
def export_mobilenetv3small_dense_multibranch_to_onnx(
    output_path: Path,
    ...
    """
    Export MobileNetV3-Small Dense Multi-Branch model to ONNX format.
    
    This function takes a MobileNetV3-Small model with dense multi-branch expert
    architecture and exports it to ONNX format, which can be used for inference on
    various platforms and devices. The exported model can be used with ONNX Runtime
    for efficient inference.
    
    Model Architecture:
    - Input: RGB image tensor (batch_size, 3, 224, 224)
    - Backbone: MobileNetV3-Small (pre-trained on ImageNet)
    - Expert Branches (parallel):
      * Each expert: Linear(576 -> 512) + Hardswish + Dropout(0.2) + Linear(512 -> num_classes)
      * Number of experts: 4 (configurable)
    - Output aggregation: Average of all expert predictions
    - Output: Class probabilities (batch_size, num_classes)
    
    The multi-branch architecture allows the model to learn multiple perspectives
    on the input data, improving generalization and robustness for plant disease
    ...
    Args:
        ...
        num_experts (int): Number of parallel expert branches. Default: 4.
    ...
    Example:
        >>> export_mobilenetv3small_dense_multibranch_to_onnx(
        ...     output_path=Path("./onnx/mobilenetv3small_dense_multibranch.onnx"),
        ...
        ... )
    """
    # Initialize model instance with specified number of experts
    model_instance = MobileNetV3SmallDenseMultiBranch(
    ...
    try:
        ...
        print(f"  - Model: MobileNetV3-Small Dense Multi-Branch")
        print(f"  - Number of experts: {num_experts}")
        print(f"  - Expert architecture: Linear(576->512)->Hardswish->Linear(512->{num_classes})")
        print(f"  - Output aggregation: Average of expert predictions")
